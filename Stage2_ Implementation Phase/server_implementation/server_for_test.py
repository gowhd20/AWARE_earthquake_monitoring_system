...
#                print 'device id : ' + arr_result[t, 0] + ' with coordinate ' + arr_result[t, 2], arr_result[t, 3] + ' was an isolated event'
        ...
        print dataToStore
        Broadcast().broadcast(centerPointArr) ## for test
        if maxCount >= 1:
...
class EventMethods:     
    def storeData(self, data_received):
        ...
        try:
            ...
            query = '''replace into client(device_id, timestamp, longitude, latitude) values (%s, %s, %s, %s)''', (data_received["device id"],data_received["timestamp"],data_received["longitude"],data_received["latitude"],data_received["acceleration"])
...
class MainThread(threading.Thread):
    def run(self):
        ...
        while(1):
            ...
            if data:
                decoded_data = Security().decode('1234567890abcdef', data)
                raw_data = JsonMethods().stripJsonObject(decoded_data)
                
                if raw_data["tag"] == 1:  ### general size -> 102, 103, 104 ###                   
                    
                   # sqlData = JsonMethods().stripJsonObject(decoded) ## for test
                    ### if first event ###
                    if on_event == 0:
                        on_event = 1
                        counter += 1
                        #sqlData = m_event.storeData(raw_data)
                        m_event.storeData(raw_data)
                        arr = array([[raw_data["device id"], raw_data["timestamp"], raw_data["latitude"], raw_data["longitude"], 1, counter]])
                        filter_id = raw_data["device id"]                    
                    ...
                    ### if time window is already opened ###
                        ...
                        if not filter_id == raw_data["device id"]:
                            counter += 1
                            #sqlData = m_event.storeData(data)
                            m_event.storeData(raw_data)
                            arr = append(arr, [[raw_data["device id"], raw_data["timestamp"], raw_data["latitude"], raw_data["longitude"], 1, counter]], axis=0)
                            filter_id = raw_data["device id"]

                elif raw_data["tag"] == 0: ### general size -> 31
                   # s_bound.sendto(securedData, addr)
                   # nkey = Security().getNewKey()

                    #print data          
                    #id_data = JsonMethods().stripJsonObject(data)
                    
                    HashFile().insertToFile(raw_data, s, addr)
                ...
                elif raw_data["tag"] == 2: ###  comment from clients
                    fobj = open(counter_file+"_.txt", "w")
                    fobj.write(raw_data["message"])
                    fobj.close()
                    counter_file+=1
